...
if(process.argv.includes('--index-only')) {
  const students={};let cursor=0;
  await Promise.all(Array.from({length:2},async()=>{
    while(cursor<roster.length) {
      const student=roster[cursor++],url=`https://api.kivo.wiki/api/v1/data/students/${student.kivoId}`;
  ...
  const catalog={schemaVersion:1,updatedAt:new Date().toISOString(),sourceFetchedAt:source.fetchedAt,students:{},files:[]};
  const tasks=new Map();
  ...
  console.log('COMPLETE',roster.length,'students,',Object.values(catalog.students).flat().length,'slots,',entries.length,'PNG files',matches);
}
